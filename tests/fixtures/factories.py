...
class VehicleFactory(factory.Factory):
    ...
    reg_number = factory.LazyAttribute(
        lambda _: f"{fake.random_letter()}{fake.random_int(100, 999)}{fake.random_letter()}{fake.random_letter()} {fake.random_int(100, 999)}"
    )
    ...
    next_service_date = factory.LazyAttribute(
        lambda _: datetime.now() + timedelta(days=fake.random_int(30, 365))
    )
...
class OrderFactory(factory.Factory):
    ...
    order_number = factory.LazyAttribute(
        lambda _: f"ORD-{fake.random_int(10000, 99999)}"
    )
    description = factory.LazyAttribute(lambda _: fake.sentence(nb_words=10))
    status = factory.LazyAttribute(
        lambda _: fake.random_element(["new", "in_progress", "completed", "cancelled"])
    )
